...
nbRuns = 15
max_pool = 32
TestSizePerNode = 2 #312500
child_seeds = ss.spawn(nbRuns*max_pool)
...
def magic_function(f):
    ...
    a=0.8
    maxKoefficient=f[1].poisson(lam=3.0, size=TestSizePerNode)+1.
    StartingValuesLocal=[f[1].uniform(low=[-1,-a,-a**2,-a**3,-a**4,-a**5,-a**6,-a**7,-a**8,-a**9],high=[1,a,a**2,a**3,a**4,a**5,a**6,a**7,a**8,a**9], size=None) for k in range(0,TestSizePerNode)]
    ...
    IncrementsBM = f[1].normal(0,1,size=[TestSizePerNode,n,10,L])
    MCprices=np.zeros(TestSizePerNode)
    for x in range(0,TestSizePerNode,1):
        price=0
        for incBM in range(0,n,1):    
            tmpvec=evalCurve(StartingValuesLocal[x],IncrementsBM[x,incBM],taugrid)
            tmp=np.sum(tmpvec)*(12/365)
            price+=max(tmp-Strike,0)
        MCprices[x]=price/n
    return StartingValuesLocal, MCprices
...
for k in range(0,nbRuns,1):
    ...
    startingValues=np.zeros((TotalTestSize,10))
    ...
    strk="% s" % k
    filename1="startingvaluesMultiVariatePoissonKoefficientTestset100000Sim"+strk
    filename2="outputsMultiVariatePoissonKoefficientTestset100000Sim"+strk
